...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#xoahet").click(function(){
      ...
      if (hoi==true) document.location = homeUrl()+"/admin/tour/"+listid+"/deleteList";
    });
  });
</script>
    ...
    <div class="col-xs-12">
      <div class="box">
          ...
          <table id="example2" class="table table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <?php  $parent = DB::table('categories_tour')->where('id', $item->cate_id)->first();
                  ?>
                  @if(!empty($parent))
                    {{ $parent->name }}
                  @else
                ...
                <td><img src="{{ asset('upload/tour/'.$item->photo) }}" onerror="this.src='{{ asset('public/admin_assets/images/no-image.jpg') }}';" class="img_product"  alt="NO PHOTO" /></td>
                <th>{{number_format($item->price)}}</th>
                <td class="text-center with_dieuhuong">
                  <div class="form-group"> 
                    @if($item->status>0)
                      <a href="admin/tour/edit?id={{$item->id}}&hienthi={{ time() }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> Hiển thị</a>
                    @else
                      <a href="admin/tour/edit?id={{$item->id}}&hienthi={{ time() }}" class="btn btn-danger btn-xs"><i class="fa fa-eye"></i> Hiển thị</a>
                    @endif
                  </div>
        ...
        <div class="box-footer col-md-12">
          <div class="row">
            <div class="col-md-6">
              <input type="button" onclick="javascript:window.location='admin/tour/create'" value="Thêm" class="btn btn-primary" />
